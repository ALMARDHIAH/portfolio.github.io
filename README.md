# Myself
Hi. I'm Ainul. My goal is to transform raw data into meaningful insights that support smarter decisions.
In this portfolio, you will see the data analytical projects that I have contributed.

# My Projects

## 2025 MST

## 2025 Cemetery Land: Sustainable Dashboard Development for Perlis E-Cemetery
During my internship, I was actively involved in the development of the Perlis e-cemetery dashboard under JAIPs (Perlis State Islamic Religious Affairs Department).
>[link](https://github.com/ALMARDHIAH/e-cemetery.github.io)

## 2025 Road Network Analysis in Federal Territory of Putrajaya
This was my final year project as part of my university course completion. Network refers to a set of nodes (road intersections) that are connected to a set of edges (road) to show the relationships between them.
>[link](https://github.com/ALMARDHIAH/road_network_analysis.github.io)

## 2022 Data Science Project: Maternal Risk
This project was part of my Introduction to Data Science course completion set by university. Data Science refers to an emerging area of work concerned with the collection, preparation, analysis, visualization, management, and preservation of large collections of information.
>[link](https://github.com/ALMARDHIAH/maternal_risk.github.io)
